...
# PLOT MAP WITH THE LOCATIONS OF ISMN STATIONS
#if __name__ == '__main__':
    sig0lia = pickle.load(open("//projectdata.eurac.edu/projects/ESA_TIGER/S1_SMC_DEV/Processing/S1ALPS/ISMN/S1AB_50m_reprocess_lt_05/sig0lia_dictNone.p",
                               'rb'), encoding='latin-1')
    sig0lia2 = pickle.load(open(
        "//projectdata.eurac.edu/projects/ESA_TIGER/S1_SMC_DEV/Processing/S1ALPS/ISMN/S1AB_50m_reprocess_lt_05_descending/sig0lia_dictNone.p",
        'rb'), encoding='latin-1')
    for ikey in sig0lia.keys():
        sig0lia[ikey].extend(sig0lia2[ikey])

    del sig0lia['plant_water_mean']
    samples = np.array(
        [sig0lia.get(x) for x in sig0lia.keys() if (x != 'sensor') and (x != 'station') and (x != 'network')],
        dtype=float)

    str_keys = ['station', 'network', 'sensor']
    station_properties = np.array([sig0lia.get(x) for x in str_keys], dtype=str)
    samples = samples.transpose()
    valLClist = [11, 14, 20, 30, 120, 140, 150]
    valid = ~np.isnan(samples).any(axis=1) & ~np.isinf(samples).any(axis=1) & (np.array(sig0lia['ssm']) > 0.0) & (
                np.array(sig0lia['ssm']) < 1) & \
            (np.array(sig0lia['gldas_swe']) == 0) & (np.array(sig0lia['gldas_soilt']) > 275)

    for ikey in sig0lia.keys():
        sig0lia[ikey] = np.array(sig0lia[ikey])[valid]
    locations_array = np.vstack((sig0lia['lon'], sig0lia['lat'])).transpose()
    unique_locations, indices, unique_counts = np.unique(locations_array, axis=0, return_index=True, return_counts=True)
    #used_stations = np.load('X:/Workspaces/GrF/Processing/S1ALPS/ISMN/gee_global/ValidStaions.npy')
    #used_stations = np.load("//projectdata.eurac.edu/projects/ESA_TIGER/S1_SMC_DEV/Processing/S1ALPS/ISMN/gee_global_no_deep_new/ValidStaions.npy")
    #used_stations = np.load(open('//projectdata.eurac.edu/projects/ESA_TIGER/S1_SMC_DEV/Processing/S1ALPS/ISMN/S1AB_250m/ValidStaions.npy', 'rb'))
    #used_stations = pickle.load(open("//projectdata.eurac.edu/projects/ESA_TIGER/S1_SMC_DEV/Processing/S1ALPS/ISMN/S1AB_100m_reprocess_lt_05/testset_meta.p", "rb"))
    #ntwks = np.array([z[0].strip() for z in [x.split(',') for x in used_stations]])
    #sttns = np.array([z[1].strip() for z in [x.split(',') for x in used_stations]])
    # ntwks = np.array(used_stations[1])
    # sttns = np.array(used_stations[0])
    # lons = np.array(used_stations[2][:,0])
    # lats = np.array(used_stations[2][:,1])

    ntwks = np.array(sig0lia['network'][indices])
    sttns = np.array(sig0lia['station'][indices])
    lons = np.array(unique_locations[:,0]).squeeze()
    lats = np.array(unique_locations[:,1]).squeeze()
    ...
    cntr = 0
    for i_ntwk in u_ntwks:
        ids = np.where(ntwks == i_ntwk)
        sub_sttns = sttns[ids]
        #sub_lons = [ismn.get_station(sub_sttns[i], i_ntwk).longitude for i in range(len(sub_sttns))]
        #sub_lats = [ismn.get_station(sub_sttns[i], i_ntwk).latitude for i in range(len(sub_sttns))]
        sub_lons = lons[ids]
        sub_lats = lats[ids]
        x, y = m(sub_lons, sub_lats)

        m.plot(x,y, marker='.', color='C' + str(cntr), linestyle='', markersize=1.5, label=str(i_ntwk))
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1),
              fancybox=True, shadow=True, ncol=3, fontsize='x-small', markerscale=5)

    #plt.show()
    plt.tight_layout()
    #plt.legend(loc=8, fontsize='small', mode='expand')
    plt.savefig("C:/Users/FGreifeneder/OneDrive - Scientific Network South Tyrol/1_THESIS/pub3/images_submission2/used_ismn_stations.png", dpi=600)
    plt.close()
